...
def test_jooble_missing_id_derives_stable_key():
    job = _load("jooble_sample.json")["jobs"][0]
    job = {**job, "id": None}
    first = jooble.normalize(job)["external_id"]
    second = jooble.normalize(job)["external_id"]
    assert first == second and len(first) == 32  # link-derived, deterministic
